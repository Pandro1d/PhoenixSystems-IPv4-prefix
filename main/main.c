...
int main() {
    ...
    //5.^Should print Mask length: 20 is in the 32.64.128.0/20 Prefix
    test_check_fun(check(&dataBase,IPv4_stringToIntConv("10.20.0.1",&status),&status), &iter);
    //6.^Should Mask length: 15 -> 10.20.0.0 is in the 10.20.0.0/16 Prefix (15 is the smallest
    // Prefix)
    test_check_fun(check(&dataBase,IPv4_stringToIntConv("10.22.0.0",&status),&status), &iter);
    //7.^Should print Mask length: -1 -> 10.22.0.0 is not in the Prefix database
    test_check_fun(check(&dataBase,IPv4_stringToIntConv("255.255.255.255",&status),&status), &iter);
    //8.^Should print Mask length: -1 -> 255.255.255.255 is not in the Prefix database
    ...
    //11.^Should print OK 12.20.0.0/20 is in the database
